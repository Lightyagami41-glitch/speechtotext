...
def translate_text():
    global showText
    text = showText.get(1.0, END)
    translator = Translator()
    translation = translator.translate(text, dest='en')
    translated_text = translation.text
    print(translated_text)
    showText.delete(1.0, END)
    showText.insert(END, translated_text)
...
def audio_to_text():
    ...
    lis.place(x= 20, y = 100)
    Button(wn2, text='Select', bg='ivory3', font=('Courier', 13),
           command=lambda:togetind()).place(x=120, y=100)
    Button(wn2, text='Start', bg='ivory3', font=('Courier', 13),
           command=takeCommand).place(x=200, y=100)


    Button(wn2, text='Stop', bg='ivory3', font=('Courier', 13),
           command=stop).place(x=280, y=100)
    Button(wn2, text='translate', bg='ivory3', font=('Courier', 13),
           command=translate_text).place(x=350, y=100)
